...
// Walks the AST and produces an HTML body. HTML-escaping happens
// here, at emit time: text-bearing nodes carry raw text and are
// escaped on output. Span tag literals live in HtmlRenderer.cpp
// (spanTags), a rendering concern.
class MARKDOWNPARSER_API HtmlRenderer : public NodeVisitor {
public:
    QString render(const Node &root);
    // Renders a list of inline nodes to their HTML (no enclosing
    // block tag).
    QString renderInlines(
...
private:
    ...
    // Collected headings (level, text, slug) for table-of-contents
    // generation.
    struct HeadingEntry {
        int level;
        QString text;
    ...
    static QString escape(const QString &raw) {
