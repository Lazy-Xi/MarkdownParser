...
// Inline-level nodes. Text-bearing nodes carry RAW (unescaped) text;
// the renderer escapes at emit time.
...
// Verbatim passthrough (e.g. backslash-escaped char already resolved
// to a literal, or raw HTML). Renderer emits as-is, no escaping.
...
// The five emphasis-like spans differ only by tag pair, which is
// renderer data.
enum class SpanKind {
    Strong,
    Emphasis,
...
// Inline code span; raw code, not inline-parsed. Renderer escapes on
// emit.
...
public:
    ImageNode(QString src, QString alt)
        : Node(NodeType::Image), src_(std::move(src)),
          alt_(std::move(alt)) {}
