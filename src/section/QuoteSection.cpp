    ...
    for (qsizetype i = 0; i < lines.size(); ++i) {
        if (std::holds_alternative<QString>(lines[i])) {
            QRegularExpressionMatch match =
                RegexUtils::quote().match(std::get<QString>(lines[i]));
            const QString &text = match.hasMatch()
                                      ? match.captured(1)
                                      : std::get<QString>(lines[i]).trimmed();
            ...
            if (text.isEmpty()) {
                ...
                if (i + 1 < lines.size() &&
                    std::holds_alternative<QString>(lines[i + 1])) {
                    content.append(" ");
                }
            }
        }
...
QString QuoteSection::before() { return QString("<blockquote>"); }

QString QuoteSection::after() { return QString("</blockquote>"); }