...
UnorderedListSection &UnorderedListSection::operator=(
    const UnorderedListSection &other) {
    if (this != &other) {
...
UnorderedListSection &UnorderedListSection::operator=(
    UnorderedListSection &&other) noexcept {
    if (this != &other) {
...
QString UnorderedListSection::toHtml() {
    ...
    for (auto &i : this->lines) {
        if (std::holds_alternative<QString>(i)) {
            ...
            const QString &text = RegexUtils::unorderedList(true)
                                      .match(std::get<QString>(i))
                                      .captured(2);
...
QString UnorderedListSection::before() { return QString("<ul>"); }

QString UnorderedListSection::after() { return QString("</ul>"); }