...
const QMap<SectionType, std::function<AbstractSection *(const RawSection &)>>
    SectionFactory::creators = {
        {SectionType::CODE_BLOCK,
            [](const RawSection &section) {
                return new CodeBlockSection(
        ...
        {SectionType::HORIZONTAL_RULE,
            [](const RawSection &section) {
                return new HorizontalRuleSection(
                    std::get<AbstractSection1::LinesType>(section.lines));
            }},
        {SectionType::ORDERED_LIST,
            [](const RawSection &section) {
                return new OrderedListSection(
        ...
        {SectionType::UNORDERED_LIST,
            [](const RawSection &section) {
                return new UnorderedListSection(
                    std::get<AbstractSection2::LinesType>(section.lines));
            }},
};

const QMap<SectionType, std::function<bool(const QString &)>>
    SectionFactory::checks = {
...
AbstractSection *SectionFactory::createSection(const RawSection &section) {
    if (creators.contains(section.type)) {
        return creators[section.type](section);
    }
...
SectionType SectionFactory::getLineType(const QString &line) {
    ...
    if (is_ordered_list || is_unordered_list) {
        return is_ordered_list ? SectionType::ORDERED_LIST
                               : SectionType::UNORDERED_LIST;
    }
