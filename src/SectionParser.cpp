...
static QStringList parseHtmlBlockLines(QStringListIterator &it) {
    ...
    while (it.hasNext()) {
        ...
        if (!lines.isEmpty() && t != SectionType::HTML &&
            t != SectionType::TEXT)
            break;
...
static std::unique_ptr<Node> buildTableNodeFromLines(
    ...
    for (const auto &s : seps) {
        ...
        aligns.append((l && r) ? Align::CENTER
                      : r      ? Align::RIGHT
                               : Align::LEFT);
    }
    ...
    for (qsizetype r = 2; r < lines.size(); ++r) {
        ...
        for (qsizetype i = 0; i < qMin(cols, cells.size()); ++i) {
            auto cell =
                std::make_unique<TableCellNode>(aligns[i], false);
...
std::unique_ptr<Node> SectionParser::nextNode() {
    ...
    const QString &line = it.peekNext();
    if (line == "<!--TOC-->") {
        // Skip the marker and any hand-maintained TOC body
        // up to the closing marker; emit a TocNode
        // placeholder for the renderer to fill.
        it.next();
        while (it.hasNext() && it.peekNext() != "<!--/TOC-->")
            it.next();
    ...
    case SectionType::INDENTED_CODE_BLOCK:
        return buildIndentedCodeBlockNode();
    case SectionType::HORIZONTAL_RULE:
    ...
    case SectionType::ORDERED_LIST:
        return buildListNode(SectionType::ORDERED_LIST);
    case SectionType::UNORDERED_LIST:
        return buildListNode(SectionType::UNORDERED_LIST);
...
std::unique_ptr<Node> SectionParser::buildParagraphNode() {
    QString content;
    while (it.hasNext()) {
        if (SectionFactory::getLineType(it.peekNext()) !=
            SectionType::TEXT)
            break;
        const QString &l = it.next();
        content += l;
...
std::unique_ptr<Node> SectionParser::tryBuildTableNode() {
    ...
    while (it.hasNext()) {
        ...
        if (TypeUtils::isTableHeaderSeparator(l)) {
            sep_break = true;
            break;
        }
        lines.append(l);
...
std::unique_ptr<Node> SectionParser::buildListNode(SectionType type) {
    ...
    const QRegularExpression &re =
        (type == SectionType::ORDERED_LIST)
            ? RegexUtils::orderedList(true)
            : RegexUtils::unorderedList(true);

    auto list =
        std::make_unique<ListNode>(type == SectionType::ORDERED_LIST);
    ...
    while (it.hasNext()) {
        ...
        bool allowed =
            lt != SectionType::TITLE && lt != SectionType::EMPTY;

        if (base == -1 && lt == type) base = ind;
        if (!allowed || ind < base ||
            (ind == base && lt == other_type[type]))
            break;
        ...
        if (ind > base) {
            ...
            while (it.hasNext()) {
                ...
                if (st == SectionType::TITLE ||
                    st == SectionType::EMPTY ||
                    indentOf(it.peekNext()) < ind)
                    break;
                sub.append(it.next().mid(ind << 1));
            }
...
std::unique_ptr<Node> SectionParser::buildQuoteNode() {
    ...
    static const QSet<SectionType> stop {
        SectionType::EMPTY, SectionType::TITLE};
    static const auto lineType = [](const QString &l) {
        return SectionFactory::getLineType(l.trimmed());
    };
    QStringList sub;
