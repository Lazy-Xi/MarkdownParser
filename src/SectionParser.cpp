...
AbstractSection1::LinesType SectionParser::parseSingleLine(SectionType type) {
    static const QSet<SectionType> single_line_types {
        SectionType::HORIZONTAL_RULE, SectionType::TITLE};
...
AbstractSection2::LinesType SectionParser::parseList(SectionType type) {
    if (type != SectionType::ORDERED_LIST &&
    ...
    while (it.hasNext()) {
        ...
        const bool is_allowed = getLineType(line) != SectionType::TITLE &&
                                getLineType(line) != SectionType::EMPTY;
        ...
        if (indent_level > base_indent_level) {
            ...
            while (it.hasNext()) {
                ...
                else if (_type == SectionType::TITLE ||
                         _type == SectionType::EMPTY ||
...
AbstractSection2::LinesType SectionParser::parseQuote() {
    static const QSet<SectionType> finished_types = {
...
AbstractSection1::LinesType SectionParser::parseHtmlBlock() {
    ...
    while (it.hasNext()) {
        ...
        if (start_tag_match.hasMatch()) {
            ...
            if (TypeUtils::isHtmlVoidTag(tag) &&
        ...
        if (end_tag_match.hasMatch() && !stack.isEmpty() &&
            stack.top() == end_tag_match.captured(1)) {
            stack.pop();
            if (stack.isEmpty()) {
                break;
            }
