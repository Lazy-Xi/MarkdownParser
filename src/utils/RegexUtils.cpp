...
const QRegularExpression &title() {
    ...
    static const QRegularExpression re(R"(^ {0,3}(#{1,6}) +(.*?)(?:\s+#+)? *$)");
    return re;
}
...
const QRegularExpression &quote() {
    // Match the quote marker both before HTML-escaping (raw '>', as seen during
    // line classification) and after (escaped '&gt;', as seen during
    // rendering).
    static const QRegularExpression re(R"(^ {0,3}(?:>|&gt;) *(.*?) *$)");
    return re;
}
